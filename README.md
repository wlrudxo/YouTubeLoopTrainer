...
Open `http://127.0.0.1:17311` to review the captured caption draft and add optional meaning, notes, and tags. The inbox opens the first item not yet added to Anki and advances after each add or discard. Discarded items stay tombstoned and are not recreated by later imports. Items can be added directly to the `English::PhraseLoop` Anki deck. Anki export is add-only: adding an already-added item again creates another note. The card plays only the audio on the front and shows the transcript, optional meaning, notes, local thumbnail, and source details on the back. All source metadata and generated media remain in the local `PhraseLoopData` folder.
